...
class ViewItemListAdapter(
    ...
    class ItemViewHolder(
        private var binding: BaseItemBinding,
        ...
        private fun resolveDownloader(): Downloader? {
            val ctx =
                (binding.root.context as? ViewComponentManager.FragmentContextWrapper)?.applicationContext
                    ?: binding.root.context.applicationContext
            return try {
                val entry =
                    dagger.hilt.android.EntryPointAccessors
                        .fromApplication(ctx, DownloaderEntryPoint::class.java)
                entry.downloader()
            } catch (_: Exception) {
                null
            }
        ...
        fun bind(
            ...
            android.util.Log.d(
                "DownloadsUI",
                "Bind item ${item.id} name=${item.name} itemView size BEFORE: w=${itemView.width} h=${itemView.height}",
            )
            itemView.post {
                android.util.Log.d(
                    "DownloadsUI",
                    "Bind item ${item.id} itemView size AFTER LAYOUT: w=${itemView.width} h=${itemView.height}",
                )
            }
            ...
            item.sources.forEachIndexed { index, source ->
                Timber.tag("DownloadsUI").d(
                    "  Source %d: type=%s path=%s downloadId=%s",
                    index,
                    source.type,
                    source.path,
                    source.downloadId,
                    ...
                    progressJob =
                        CoroutineScope(Dispatchers.Main).launch {
                            while (isActive) {
                                ...
                                Timber.tag("DownloadsUI").d(
                                    "Download progress: %d%% (status=%d, downloadId=%s)",
                                    progress,
                                    status,
                                    source?.downloadId,
                                )
