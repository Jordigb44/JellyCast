    ...
    constructor(
        ...
        fun loadPreferences(
            ...
            viewModelScope.launch {
                ...
                for (index in indexes) {
                    ...
                    val preference =
                        preferences.flatMap { it.preferences }.filterIsInstance<PreferenceCategory>().find {
                    ...
                    preferences
                        .map { preferenceGroup ->
                            preferenceGroup.copy(
                                preferences =
                                    preferenceGroup.preferences.filter { it.supportedDeviceTypes.contains(deviceType) }.map { preference ->
                                        when (preference) {
                                            is PreferenceSwitch -> {
                                                preference.copy(
                                                    enabled =
                                                        preference.enabled && preference.dependencies.all { appPreferences.getValue(it) },
        ...
        fun updateExternalPlayerDescription(playerName: String?) {
            viewModelScope.launch {
                ...
                val updatedGroups =
                    currentState.preferenceGroups.map { group ->
                        group.copy(
                            preferences =
