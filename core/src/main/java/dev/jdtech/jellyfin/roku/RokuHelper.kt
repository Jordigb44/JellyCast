...
object RokuHelper {
     ...
     */
    suspend fun discoverRokuDevices(context: Context): List<RokuDevice> =
     ...
     */
    private suspend fun parseRokuDeviceInfo(location: String): RokuDevice? =
        withContext(Dispatchers.IO) {
            try {
     ...
     */
    fun isRokuDeviceAvailable(): Boolean = currentDevice != null
     ...
     */
    suspend fun sendKeyPress(key: String): Boolean =
        withContext(Dispatchers.IO) {
            val device = currentDevice ?: return@withContext false
     ...
     */
    suspend fun stopPlayback(): Boolean = sendKeyPress("Home")

    /**
     * Pause/Resume playback on Roku
     */
    suspend fun pauseResume(): Boolean = sendKeyPress("Play")
